...
class SynBlockData {
    ...
    try {
            //目前按一个channel做同步
            console.log("this is burrow SynBlockData");
            var curBlockNum;
            var genesisBlockHash = "";
            var channelName = "";
            var statusdata = JSON.parse(platform.getStatus());
            if(statusdata) {
               genesisBlockHash = statusdata.result.NodeInfo.Network;
               channelName = statusdata.result.NodeInfo.Network;
            }
    ...
    async getBlockTimeStamp(block) {
        ...
        try {
            if (block && block.BlockMeta && block.BlockMeta.header.time) {
                blockTimestamp = block.BlockMeta.header.time;
            }
        } catch (err) {
            logger.error(err)
        }
...
/*        if (blockSaved) {
    ...
    async saveTransactions(block,channelName) {
        ...
        //synccontracts();
        //////////tx////////////////////////
        let txLen = block.BlockMeta.header.num_txs;
        if (parseInt(txLen) >0 ) {
          for (let i = 0; i < txLen; i++) {
            var tx = block.block.data.txs[i];
            var txhash = this.calculateTxHash(tx);
            var transaction = {
                'blockid': block.BlockMeta.header.height,
                'txhash': txhash,
                'createdt': new Date(block.BlockMeta.header.time),
                'contractname': "",
                'contract_id': "",
                'status': 0,
                'creator_msp_id': "",
    ...
    async getMaxBlockNum(channelName) {
        try {
            ...
            var height = "";
            if(statusdata) {
               height = statusdata.result.SyncInfo.LatestBlockHeight;
            }
    ...
    async saveChannel() {
            ...
            var chain_id = "";
            var total_txs = "";
            console.log(12345,statusdata);
            if(statusdata) {
               nums = statusdata.result.SyncInfo.LatestBlockHeight;
               chain_id = statusdata.result.NodeInfo.Network;
            }
            //从最后一块内，获取交易总数
            var blockdata =platform.getBlockByNumber(chain_id, nums);
            if(blockdata) {
                total_txs = blockdata.BlockMeta.header.total_txs;
            }
            let date = new Date()
            var channel = {
    ...
    async saveNodelist() {
        ...
        var netinfo = JSON.parse(platform.getnetInfo());
         ...
         if(statusdata) {
            nodelists.push({"requests": statusdata.result.NodeInfo.ListenAddress,
                            "genesis_block_hash": statusdata.result.NodeInfo.Network,
                            "server_hostname": statusdata.result.NodeInfo.ListenAddress
                        });
        }
        if(netinfo) {
            var netnodes = netinfo.result.peers;
            for( var i = 0;i < netnodes.length;i++){
                nodelists.push({"requests": netnodes[i].Node.listen_addr,
                                "genesis_block_hash": netnodes[i].Node.network,
                                "server_hostname": netnodes[i].Node.listen_addr
                              });
            }
        }
